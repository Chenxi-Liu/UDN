...
%         .*exp(pi*lambda_bs/c.*(exp(-c.*x.^2)-exp(-c.*x.^(2*alpha_l/alpha_nl))))...
%         .*(alpha_l/alpha_nl.*x.^(2*alpha_l/alpha_nl-1)-alpha_l/alpha_nl.*x.^(2*alpha_l/alpha_nl-1).*exp(-c.*x.^(2*alpha_l/alpha_nl))+x.*exp(-c.*x.^2))...
%         .*exp(-2*pi*lambda_bs.*arrayfun(@(xx)integral(@(y)(1-1./((1+r_th/m.*(xx^2+h^2)^(alpha_l/2).*(y.^2+h^2).^(-alpha_l/2))).^m).*exp(-c.*y.^2).*y,xx,inf),x))...
%         .*exp(-2*pi*lambda_bs.*arrayfun(@(xx)integral(@(z) (1-1./(1+r_th*(xx^2+h^2)^(alpha_l/2).*(z.^2+h^2).^(-alpha_nl/2))).*(1-exp(-c.*z.^2)).*z,xx^(alpha_l/alpha_nl),inf),x)),0,inf);
    
        part_2 = 1-integral(@(x)(1-exp(-c.*x.^2)).*2*pi*lambda_bs.*exp(-pi*lambda_bs.*x.^(2*alpha_l/alpha_nl))...
        .*exp(pi*lambda_bs/c.*(exp(-c.*x.^2)-exp(-c.*x.^(2*alpha_l/alpha_nl))))...
        .*(alpha_l/alpha_nl.*x.^(2*alpha_l/alpha_nl-1)-alpha_l/alpha_nl.*x.^(2*alpha_l/alpha_nl-1).*exp(-c.*x.^(2*alpha_l/alpha_nl))+x.*exp(-c.*x.^2))...
        .*exp(-pi*lambda_bs.*exp(c*h^2).*arrayfun(@(xx)integral(@(y)(1-1./((1+r_th/m.*(xx^2+h^2)^(alpha_nl/2).*(y).^(-alpha_l/2))).^m).*exp(-c.*y),xx^2+h^2,inf),x))...
        .*exp(-pi*lambda_bs.*exp(c*h^2).*arrayfun(@(xx)integral(@(z) (1-1./(1+r_th*(xx^2+h^2)^(alpha_nl/2).*(z).^(-alpha_nl/2))).*(1-exp(-c.*z)),xx^(2*alpha_l/alpha_nl)+h^2,inf),x)),0,inf)...
        -integral(@(x)(exp(-c.*x.^2)).*2*pi*lambda_bs.*exp(-pi*lambda_bs.*x.^(2*alpha_l/alpha_nl))...
        .*exp(pi*lambda_bs/c.*(exp(-c.*x.^2)-exp(-c.*x.^(2*alpha_l/alpha_nl))))...
        .*(alpha_l/alpha_nl.*x.^(2*alpha_l/alpha_nl-1)-alpha_l/alpha_nl.*x.^(2*alpha_l/alpha_nl-1).*exp(-c.*x.^(2*alpha_l/alpha_nl))+x.*exp(-c.*x.^2))...
        .*exp(-pi*lambda_bs.*exp(c*h^2).*arrayfun(@(xx)integral(@(y)(1-1./((1+r_th/m.*(xx^2+h^2)^(alpha_l/2).*(y).^(-alpha_l/2))).^m).*exp(-c.*y),xx^2+h^2,inf),x))...
        .*exp(-pi*lambda_bs.*exp(c*h^2).*arrayfun(@(xx)integral(@(z) (1-1./(1+r_th*(xx^2+h^2)^(alpha_l/2).*(z).^(-alpha_nl/2))).*(1-exp(-c.*z)),xx^(2*alpha_l/alpha_nl)+h^2,inf),x)),0,inf);
